...
t_bmp24 *bmp24_rotate90c(t_bmp24 *imagec) {
    ...
    t_bmp24 *rotated = bmp24_allocate(imagec->height, imagec->width, imagec->colorDepth);
    if (rotated == NULL) {
...
int bmp24_validateHeader(t_bmp24 *image) {
    ...
    // Check the BMP type
    if (image->header.type != BMP_TYPE) {
        printf("Invalid BMP type: expected 0x4D42, got 0x%.4X\n", image->header.type);
        return -1;
    }

    // Check the size of the header
    printf("Invalid header size: expected %lu, got %lu\n", 
        (long unsigned int)(sizeof(t_bmp_header) + sizeof(t_bmp_info) + 
        (image->header_info.width * image->header_info.height * (image->colorDepth / 8))),
        (long unsigned int)image->header.size);

    // Check the offset for pixel data
    printf("Invalid pixel data offset: expected %lu, got %lu\n", 
        (long unsigned int)(sizeof(t_bmp_header) + sizeof(t_bmp_info)), 
        (long unsigned int)image->header.offset);

    // Check width and height
    if (image->header_info.width <= 0 || image->header_info.height <= 0) {
        printf("Invalid image dimensions: width=%d, height=%d\n",
            image->header_info.width, image->header_info.height);
        return -1;
    }

    // Check the color depth
    if (image->colorDepth != DEFAULT_DEPTH) {
        printf("Invalid color depth: expected %d, got %d\n", DEFAULT_DEPTH, image->colorDepth);
        return -1;
    }
